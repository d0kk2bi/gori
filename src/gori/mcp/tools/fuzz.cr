    ...
    class Tools
      ...
      private def list_fuzz_tools(j : JSON::Builder) : Nil
        ...
        tool j, "fuzz_start",
          ...
          s.field "no_encode", boolprop("send payloads into query-string / form-body positions RAW — turns off the default percent-encoding for those positions (path, JSON body, header and cookie positions are raw either way). For a payload that IS the raw byte: parameter pollution with a bare &, a request-line CRLF probe. An explicit 'processors' pipeline already replaces the default.")
