...
describe Gori::Tui::SetupWizard do
  ...
  # The PET step holds Miss Ring's column band back out of its own text column, so a card too
  ...
  describe ".pet_preview_x" do
    # PET is a non-BIND step, so it gets the wide card.
    private_box = ->(w : Int32) {
      cw = SW.card_w(w, 84)
      Gori::Tui::Rect.new({(w - cw) // 2, 0}.max, 0, cw, SW.card_h(SW::MIN_H, SW::PET_ROWS))
    }

    it "drops her at the advertised minimum width" do
      SW.pet_preview_x(private_box.call(SW::MIN_W)).should be_nil
    end

    it "seats her on a card that can hold the step's opening line beside her" do
      # 69 columns is the first width whose card leaves PET_TEXT_MIN for the text, and 68 the
      # last that doesn't — the pair is what stops the constant from being quietly padded.
      # PET_TEXT_MIN is the width of that opening line ("A mascot in the corner, off unless
      ...
      SW.pet_preview_x(private_box.call(69)).should_not be_nil
      SW.pet_preview_x(private_box.call(68)).should be_nil
      SW.pet_preview_x(private_box.call(80)).should_not be_nil
    end

    it "leaves her sprite inside the card's interior" do
      # She occupies `px .. px + PET_PREVIEW_W - 1` (plate, sprite, plate); the interior's last
      # column is `right - 2`. draw_pet_preview writes those cells unclipped, so an off-by-one
      # here would paint over the card's right border rather than truncate.
      box = private_box.call(80)
      px = SW.pet_preview_x(box).not_nil!
      (px + SW::PET_PREVIEW_W - 1).should be <= box.right - 2
      px.should be > box.x
    end
  end
end
