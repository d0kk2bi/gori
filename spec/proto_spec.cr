...
describe Gori::Proto do
  describe ".classify" do
    it "classifies a 101 upgrade as WebSocket (status wins over any type)" do
      ...
      Gori::Proto.classify(101, "application/grpc", nil, nil).should eq(Gori::Proto::Kind::Ws)
    end

    it "classifies gRPC by Content-Type, including +proto and grpc-web variants" do
      Gori::Proto.classify(200, "application/grpc", nil, nil).should eq(Gori::Proto::Kind::Grpc)
      Gori::Proto.classify(200, "application/grpc+proto", nil, nil).should eq(Gori::Proto::Kind::Grpc)
      Gori::Proto.classify(200, "application/grpc-web+proto", nil, nil).should eq(Gori::Proto::Kind::Grpc)
      Gori::Proto.classify(200, "APPLICATION/GRPC", nil, nil).should eq(Gori::Proto::Kind::Grpc)
    end
    ...
    it "treats everything else — including a pending/typeless flow — as HTTP" do
      Gori::Proto.classify(200, "text/html", nil, nil).should eq(Gori::Proto::Kind::Http)
      Gori::Proto.classify(nil, nil, nil, nil).should eq(Gori::Proto::Kind::Http)
      Gori::Proto.classify(200, nil, nil, nil).should eq(Gori::Proto::Kind::Http)
    end
    ...
    it "classifies gRPC from the REQUEST's type when the response has none or the wrong one" do
      Gori::Proto.classify(nil, nil, "application/grpc", nil).should eq(Gori::Proto::Kind::Grpc)         # still pending
      Gori::Proto.classify(502, "text/html", "application/grpc", nil).should eq(Gori::Proto::Kind::Grpc) # a proxy error page
      Gori::Proto.classify(200, nil, "application/grpc-web+proto", nil).should eq(Gori::Proto::Kind::Grpc)
      Gori::Proto.classify(nil, nil, "APPLICATION/GRPC; charset=utf-8", nil).should eq(Gori::Proto::Kind::Grpc)
    end

    it "still lets the 101 handshake win over a gRPC request type" do
      Gori::Proto.classify(101, nil, "application/grpc", nil).should eq(Gori::Proto::Kind::Ws)
    end
    ...
    it "does not read SSE off the request type" do
      Gori::Proto.classify(200, "text/html", "text/event-stream", nil).should eq(Gori::Proto::Kind::Http)
    end

    it "leaves an ordinary request type alone" do
      Gori::Proto.classify(200, "text/html", "application/json", nil).should eq(Gori::Proto::Kind::Http)
    end
  end
