...
**필터링할 때 걸리는 주의점이 하나 있습니다.** gori는 여전히 HTTP/1.1 핸드셰이크의 `101` 상태로 PROTO 열과 `proto:ws` 필터를 판정합니다. RFC 8441 소켓의 응답은 `200`이므로 History에서 `HTTPS`로 표시되고 **`proto:ws`로는 잡히지 않습니다.** MESSAGES 패널에 메시지가 그대로 보이는데도 그렇습니다. [#743](https://github.com/hahwul/gori/issues/743)이 반영되기 전까지는 메서드로 거르세요. HTTP/2 위의 `CONNECT`가 곧 확장 CONNECT입니다.
