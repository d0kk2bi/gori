...
### 프로젝트를 넘어 규칙 재사용하기 {#reusing-a-rule-across-projects}

CSP 헤더 제거, `X-Forwarded-For` 고정, 라이선스 확인 스텁처럼 매번 다시 만들게 되는 규칙은 모든 프로젝트가 공유하는 **라이브러리**에 넣어 둘 수 있습니다. 규칙 목록의 space 메뉴에서:

- **Save rule to library** 는 선택한 규칙에 이름을 붙여 설정의 `rewriter` 섹션에 씁니다(입력창은 규칙 이름으로 채워져 열리고, 이미 있는 이름으로 저장하면 그 항목을 갱신합니다).
- **Load a saved rule** 는 라이브러리 피커를 열어 이름 옆에 매칭과 치환 내용을 함께 보여 주고, 고른 항목을 이 프로젝트에 **추가**합니다. 타이핑으로 걸러 볼 수 있고, `Ctrl-X` 는 선택한 항목을 라이브러리에서 지웁니다. 이미 프로젝트에 불러온 규칙은 건드리지 않습니다.

불러온 규칙은 적용 순서의 맨 뒤에 붙고 추가/복제와 마찬가지로 **켜진 상태**로 들어오므로 곧바로 트래픽을 재작성합니다. `Shift-J`/`Shift-K` 로 순서를 옮기거나 `x` 로 끄면 됩니다. 라이브러리는 항목 하나에 규칙 하나를 담고, 이미 있는 목록을 교체하거나 병합하지 않습니다. 옮겨 가는 것은 규칙 자체의 필드뿐이고, 어떤 규칙이 어떤 순서로 켜져 있는지는 프로젝트에 남습니다.
