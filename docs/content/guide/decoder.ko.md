...
이름을 붙인 체인은 설정의 `decoder` 섹션에 저장되어 모든 프로젝트에서 공유됩니다. 체인은 조리법이고, 거기에 통과시킨 내용은 프로젝트에 남습니다. Rewriter 도 [규칙 라이브러리](/ko/guide/proxy/#reusing-a-rule-across-projects)를 같은 방식으로 관리합니다.
