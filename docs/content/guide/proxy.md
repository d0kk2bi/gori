...
**One caveat that will bite you when filtering.** gori still decides the PROTO column and the `proto:ws` filter from the HTTP/1.1 handshake's `101` status. An RFC 8441 socket is answered `200`, so it shows as `HTTPS` in History and **`proto:ws` does not match it**, even though its messages are right there in the MESSAGES pane. Filter these by method instead — a `CONNECT` on HTTP/2 is an extended CONNECT — until [#743](https://github.com/hahwul/gori/issues/743) lands.
